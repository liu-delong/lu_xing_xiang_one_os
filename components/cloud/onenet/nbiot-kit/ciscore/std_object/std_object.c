...
#if CIS_ENABLE_DM
extern cis_coapret_t std_dm666_read(st_context_t * contextP,uint16_t instanceId,int * numDataP,st_data_t * * dataArrayP,st_object_t * objectP);
extern cis_coapret_t std_dm666_discover(st_context_t * contextP,uint16_t instanceId,int * numDataP,st_data_t * * dataArrayP,st_object_t * objectP);
#endif //CIS_ENABLE_DM
...
#if CIS_ENABLE_DM
	{ CIS_DM666_OBJECT_ID,std_dm666_read,NULL,NULL,std_dm666_discover },
#endif
...
#if CIS_ENABLE_DM
cis_list_t* std_object_get_dm666(st_context_t * contextP, cis_iid_t instanceId)
{
	contextP->dm666_inst = CIS_LIST_FIND(contextP->dm666_inst, instanceId);
	return contextP->dm666_inst;
}

cis_list_t* std_object_put_dm666(st_context_t * contextP, cis_list_t* targetP)
{
	contextP->dm666_inst = CIS_LIST_ADD(contextP->dm666_inst, targetP);
	return contextP->dm666_inst;
}

void std_object_remove_dm666(st_context_t * contextP, cis_list_t* targetP)
{
	contextP->dm666_inst = CIS_LIST_RM(contextP->dm666_inst, targetP->id, NULL);
}
#endif
