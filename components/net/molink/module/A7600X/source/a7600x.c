...
{
    mo_a7600x_t *module = (mo_a7600x_t *)malloc(sizeof(mo_a7600x_t));
    if (OS_NULL == module)
    {
...
int a7600x_auto_create(void)
{
    ...
    if (OS_NULL == device)
    {
        LOG_EXT_E("Auto create failed, Can not find %s interface device!", A7600X_DEVICE_NAME);
        return OS_ERROR;
    }
    ...
    if (OS_NULL == module)
    {
        LOG_EXT_E("Auto create failed, Can not create %s module object!", A7600X_NAME);
        return OS_ERROR;
    }

    LOG_EXT_I("Auto create %s module object success!", A7600X_NAME);
    return OS_EOK;
}
OS_CMPOENT_INIT(a7600x_auto_create);
