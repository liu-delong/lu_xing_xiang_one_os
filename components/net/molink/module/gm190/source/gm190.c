...
{
    mo_gm190_t *module = (mo_gm190_t *)malloc(sizeof(mo_gm190_t));
    if (OS_NULL == module)
    {
...
int gm190_auto_create(void)
{
    ...
    if (OS_NULL == device)
    {
        LOG_EXT_E("Auto create failed, Can not find M5311 interface device %s!", GM190_DEVICE_NAME);
        return OS_ERROR;
    }
    ...
    if (OS_NULL == module)
    {
        LOG_EXT_E("Auto create failed, Can not create %s module object!", GM190_NAME);
        return OS_ERROR;
    }

    LOG_EXT_I("Auto create %s module object success!", GM190_NAME);
    return OS_EOK;
}
OS_CMPOENT_INIT(gm190_auto_create);
