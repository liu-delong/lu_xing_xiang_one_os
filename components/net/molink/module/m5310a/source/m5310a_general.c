...
os_err_t m5310a_clear_stored_earfcn(mo_object_t *self)
{
    ...
    return at_parser_exec_cmd(parser, "AT+NCSEARFCN");
}
...
os_err_t m5310a_get_app_version(mo_object_t *self, char *value, os_size_t len)
{
    ...
    at_parser_t *parser = &self->parser;
    at_parser_set_resp(parser, 256, 0, os_tick_from_ms(2000));
    os_err_t result = at_parser_exec_cmd(parser, "AT+CGMR");
    if (result != OS_EOK)
    {
    ...
    const char *version_begin = at_parser_get_line_by_kw(parser, "APPLICATION_A,");
