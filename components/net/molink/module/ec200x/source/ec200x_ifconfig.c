 ...
 *
 * @file        ec200x_ifconfig.c
 *
 * @brief       ec200x module link kit ifconfig api implement
 *
 * @revision
 * Date         Author          Notes
...
#define DBG_EXT_TAG "ec200x.ifconfig"
#define DBG_EXT_LVL DBG_EXT_INFO
#include <os_dbg_ext.h>

#ifdef EC200X_USING_IFCONFIG_OPS

os_err_t ec200x_ifconfig(mo_object_t *self)
{
    char ipaddr[IPADDR_MAX_STR_LEN + 1] = {0};
    if (ec200x_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (ec200x_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (ec200x_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
    ...
    if (ec200x_get_csq(self, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
    }
