...
os_err_t ml302_ping(mo_object_t *self, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    ...
    {
        LOG_EXT_E("ML302 ping: ping package len[%d] is out of rang[%d, %d].", 
    ...
    {
        LOG_EXT_E("ML302 ping: ping timeout %us is out of rang[%ds, %ds].", 
                  timeout_s, ML302_MIN_PING_TIMEOUT, ML302_MAX_PING_TIMEOUT);
        return OS_ERROR;
    }

    LOG_EXT_D("ML302 ping: %s, len: %d, timeout: %us", host, len, timeout_s);
    ...
    {
        LOG_EXT_D("ML302 ping: resp prase ip[%s], req_time[%u], ttl[%d]", ipaddr, req_time, ttl);
        if (ttl <= 0)
        {
            result = OS_ETIMEOUT;
        }
