...
#define DBG_EXT_TAG "bc95.netconn"
...
#ifndef BC95_DATA_QUEUE_SIZE
#define BC95_DATA_QUEUE_SIZE (5)
#endif
...
mo_netconn_t *bc95_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    os_data_queue_init(&netconn->data_queue, BC95_DATA_QUEUE_SIZE, 0, OS_NULL);
...
os_err_t bc95_netconn_destroy(mo_object_t *module, mo_netconn_t *netconn)
{
    ...
    os_int32_t   connid = BC95_CONN_ID_NULL;
...
os_err_t bc95_netconn_gethostbyname(mo_object_t *self, const char *domain_name, ip_addr_t *addr)
{
    ...
    if (OS_EOK != result)
    {
        LOG_EXT_E("BC95 domain resolve: execute error, host: %s", domain_name);
        goto __exit;
    }
    ...
    {
        LOG_EXT_E("BC95 domain resolve: resp parse fail, try again, host: %s", domain_name);
        result = OS_ERROR;
    ...
    {
        LOG_EXT_E("BC95 domain resolve: recvip len < IPADDR_MIN_STR_LEN, len = %d", strlen(recvip));
        result = OS_ERROR;
        goto __exit;
    }
    else
    {
        LOG_EXT_D("BC95 domain resolve: \"%s\" domain ip is %s, addrlen %d", domain_name, recvip, strlen(recvip));
...
static os_size_t bc95_tcp_send(at_parser_t *parser, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    os_err_t   result       = OS_EOK;
...
static os_size_t bc95_udp_send(at_parser_t *parser, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    os_err_t   result       = OS_EOK;
    os_size_t  sent_size    = 0;
    os_size_t  cur_pkt_size = 0;
...
os_size_t bc95_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    {
    case NETCONN_TYPE_TCP:
        sent_size = bc95_tcp_send(parser, netconn, hexstr, strlen(hexstr));
        break;
    case NETCONN_TYPE_UDP:
		sent_size = bc95_udp_send(parser, netconn, hexstr, strlen(hexstr));
        break;
    default:
        break;
    }
