...
static os_err_t bc95_ping_dns(mo_object_t *self, const char *domain_name, ip_addr_t *addr);

os_err_t bc95_ping(mo_object_t *self, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    ...
    {
        LOG_EXT_E("BC95 ping: ping package len[%d] is out of rang[%d, %d].", 
    ...
    {
        LOG_EXT_E("BC95 ping: user set ping timeout value %ums is out of rang[%dms, %dms].", 
                  timeout, BC95_MIN_PING_TIMEOUT, BC95_MAX_PING_TIMEOUT);
        return OS_ERROR;
    }

    LOG_EXT_D("BC95 ping: %s, len: %u, timeout: %ums", host, len, timeout);
    
    /* DNS: bc95 dosen't support ping domain name, do dns before ping */
    result = bc95_ping_dns(self, host, &addr);
    if (OS_EOK != result)
    {
        LOG_EXT_E("BC95 ping: dns error, host: %s", host);
        return result;
    }

    strncpy(ipsend, inet_ntoa(addr), IPADDR_MAX_STR_LEN);
    LOG_EXT_D("BC95 ping: domain ip is %s, addrlen %d", ipsend, strlen(ipsend));
    /* DNS: finished */
    ...
    {
        LOG_EXT_D("BC95 ping: resp prase ip[%s], req_time[%u], ttl[%hd]", iprecv, req_time, ttl);
        if (ttl <= 0)
        {
            result = OS_ETIMEOUT;
        }
