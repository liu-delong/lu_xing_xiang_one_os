...
#define DBG_EXT_TAG "bc95.ifconfig"
#define DBG_EXT_LVL DBG_EXT_INFO
#include <os_dbg_ext.h>

#ifdef BC95_USING_IFCONFIG_OPS

os_err_t bc95_ifconfig(mo_object_t *self)
{
    char ipaddr[IPADDR_MAX_STR_LEN + 1] = {0};
    if (bc95_get_ipaddr(self, ipaddr) != OS_EOK)
    {
        memset(ipaddr, 0, sizeof(ipaddr));
    }

    char imei[MO_IMEI_LEN + 1] = {0};
    if (bc95_get_imei(self, imei, sizeof(imei)) != OS_EOK)
    {
        memset(imei, 0, sizeof(imei));
    }

    char iccid[MO_ICCID_LEN + 1] = {0};
    if (bc95_get_iccid(self, iccid, sizeof(iccid)) != OS_EOK)
    {
    ...
    if (bc95_get_csq(self, &rssi, &ber) != OS_EOK)
    {
        rssi = 0;
        ber  = 0;
    }
...
os_err_t bc95_get_dnsserver(mo_object_t *self, dns_server_t *dns)
{
    ...
    {
        LOG_EXT_E("BC95 %s: get secondary dns failed.", __FUNCTION__);
        result = OS_ERROR;
        goto __exit;
    }
