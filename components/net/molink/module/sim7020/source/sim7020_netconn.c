...
#define DBG_EXT_TAG "sim7020.netconn"
#define DBG_EXT_LVL DBG_EXT_INFO
#include <os_dbg_ext.h>

#define SEND_DATA_MAX_SIZE (512)

#ifndef SIM7020_DATA_QUEUE_SIZE
#define SIM7020_DATA_QUEUE_SIZE (5)
#endif
...
mo_netconn_t *sim7020_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    os_data_queue_init(&netconn->data_queue, SIM7020_DATA_QUEUE_SIZE, 0, OS_NULL);
...
os_err_t sim7020_netconn_gethostbyname(mo_object_t *self, const char *domain_name, ip_addr_t *addr)
{
    ...
    {
        LOG_EXT_E("SIM7020 domain resolve: resp parse fail, try again, host: %s", domain_name);
        result = OS_ERROR;
    ...
    {
        LOG_EXT_E("SIM7020 domain resolve: recvip len < IPADDR_MIN_STR_LEN, len = %d", strlen(recvip));
        result = OS_ERROR;
        goto __exit;
    }
    else
    {
        LOG_EXT_D("SIM7020 domain resolve: \"%s\" domain ip is %s, addrlen %d", domain_name, recvip, strlen(recvip));
...
static os_size_t sim7020_tcp_udp_send(at_parser_t *parser, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    os_err_t   result       = OS_EOK;
    ...
    while (sent_size < size)
    {
        ...
        /* SIM7020 sends data AT command far beyond the normal length of AT command, */
        /* using a special process to execute */

        /* Protect the SIM7020 data sending process, prevent other threads to send AT commands */
...
os_size_t sim7020_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    sent_size = sim7020_tcp_udp_send(parser, netconn, hexstr, strlen(hexstr));
