...
#define DBG_EXT_TAG "sim7020.ping"
...
os_err_t sim7020_ping(mo_object_t *self, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    ...
    if (parser == OS_NULL)
    {
        LOG_EXT_E("SIM7020 ping: at parser is NULL.");
        return OS_ERROR;
    }

    if (len > SIM7020_MAX_PING_PKG_LEN)
    {
        LOG_EXT_E("SIM7020 ping: ping package len[%d] is out of rang[%d, %d].", 
                  len,
    ...
    {
        LOG_EXT_E("SIM7020 ping: user set ping timeout value %ums is out of rang[%dms, %dms].", 
                  timeout * 100,
    ...
    LOG_EXT_D("SIM7020 ping: %s, len: %d, timeout: %ums", host, len, timeout * 100);
    ...
    {
        LOG_EXT_D("SIM7020 ping: resp prase ip[%s], req_time[%d], ttl[%d]", ipaddr, req_time, ttl);
        if (ttl <= 0)
        {
            result = OS_ETIMEOUT;
        }
