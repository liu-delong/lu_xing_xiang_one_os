...
{
    mo_sim7020_t *module = (mo_sim7020_t *)malloc(sizeof(mo_sim7020_t));
    if (OS_NULL == module)
    {
...
int sim7020_auto_create(void)
{
    ...
    if (OS_NULL == device)
    {
        LOG_EXT_E("Auto create failed, Can not find SIM7020 interface device %s!", SIM7020_DEVICE_NAME);
        return OS_ERROR;
    }
    ...
    if (OS_NULL == module)
    {
        LOG_EXT_E("Auto create failed, Can not create %s module object!", SIM7020_NAME);
        return OS_ERROR;
    }

    LOG_EXT_I("Auto create %s module object success!", SIM7020_NAME);
    return OS_EOK;
}
OS_CMPOENT_INIT(sim7020_auto_create);
