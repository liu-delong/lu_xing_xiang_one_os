...
{
    mo_gm510_t *module = (mo_gm510_t *)malloc(sizeof(mo_gm510_t));
    if (OS_NULL == module)
    {
...
int gm510_auto_create(void)
{
    ...
    if (OS_NULL == device)
    {
        LOG_EXT_E("Auto create failed, Can not find M5311 interface device %s!", GM510_DEVICE_NAME);
        return OS_ERROR;
    }
    ...
    if (OS_NULL == module)
    {
        LOG_EXT_E("Auto create failed, Can not create %s module object!", GM510_NAME);
        return OS_ERROR;
    }

    LOG_EXT_I("Auto create %s module object success!", GM510_NAME);
    return OS_EOK;
}
OS_CMPOENT_INIT(gm510_auto_create);
