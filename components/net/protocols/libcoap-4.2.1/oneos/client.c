...
#define DBG_EXT_TAG "coap.client"
#define DBG_EXT_LVL LOG_LVL_DEBUG
...
static int coap_getopt(int argc, char *argv[], char *opts)
{
    ...
    if (*++cp == ':') {
        if (argv[coap_optind][sp+1] != '\0')
            ...
            LOG_EXT_E( ": option requires an argument -- %c", c);
            sp = 1;
            return '?';
        } else
...
coap_new_request(coap_context_t *ctx,
  ...
  if ( !coap_add_token(pdu, the_token.length, the_token.s)) {
    LOG_EXT_D( "cannot add token to request");
  }
...
clear_obs(coap_context_t *ctx, coap_session_t *session) {
  ...
  if (!coap_add_option(pdu,
      ...
      buf)) {
    LOG_EXT_E( "cannot add option Observe: %u", COAP_OBSERVE_CANCEL);
    goto error;
  }
  ...
  if (tid == COAP_INVALID_TID)
    LOG_EXT_D( "clear_obs: error sending new request");
...
message_handler(struct coap_context_t *ctx,
  ...
  LOG_EXT_D( "** process incoming %d.%02d response:",
  ...
  if (COAP_RESPONSE_CLASS(received->code) == 2) {
    ...
    if (!obs_started && coap_check_option(received, COAP_OPTION_OBSERVE, &opt_iter)) {
      LOG_EXT_D("observation relationship established, set timeout to %d",
               obs_seconds);
      obs_started = 1;
    ...
    if (block_opt) { /* handle Block2 */
      ...
      if (coap_get_data(received, &len, &databuf))
        ...
        LOG_EXT_E("get_data-%d:%s", len, databuf);
      ...
      if(COAP_OPT_BLOCK_MORE(block_opt)) {
        /* more bit is set */
        LOG_EXT_D( "found the M bit, block size is %u, block nr. %u",
        ...
        if ( pdu ) {
          ...
          if (tid == COAP_INVALID_TID) {
            LOG_EXT_D( "message_handler: error sending new request");
          } else {
      ...
      if (block_opt) { /* handle Block1 */
        ...
        if (szx != block.szx) {
          ...
          if (bytes_sent % (1 << (szx + 4)) == 0) {
            ...
            block.szx = szx;
            LOG_EXT_D("new Block1 size is %u, block number %u completed",
                     (1 << (block.szx + 4)), block.num);
          } else {
            LOG_EXT_D( "ignoring request to increase Block1 size, "
       ...
       /* create pdu with request for next block */
        ...
        if (pdu) {
          ...
          LOG_EXT_D( "send block %d", block.num);
          coap_add_option(pdu,
                          COAP_OPTION_BLOCK1,
                          coap_encode_var_safe(buf, sizeof(buf),
...
cmdline_content_type(char *arg, uint16_t key) {
  ...
  if (isdigit(*arg)) {
    ...
    if (content_types[i].media_type) {
      ...
      LOG_EXT_W( "W: unknown content-format '%s'",arg);
      return;
    }
  }
...
#define BUFSIZE 100
  ...
  if (proxy.length) {   /* create Proxy-Uri from argument */
    ...
    if (coap_split_uri((unsigned char *)arg, strlen(arg), &uri) < 0) {
      LOG_EXT_E( "invalid CoAP URI:%s", arg);
      return -1;
    }
    ...
    if (uri.path.length) {
      ...
      if (uri.path.length > BUFSIZE)
        LOG_EXT_W( "URI path will be truncated (max buffer %d)", BUFSIZE);
...
convert_hex_string(const char *src, uint8_t *dst) {
  ...
  if (src[0] != '\0') { /* error in hex input */
    LOG_EXT_W( "invalid hex string in option '%s'", src);
  }
  return p - dst;
}
...
coap_client_main(int argc, char **argv) {
  ...
  if ( ( user_length < 0 ) || ( key_length < 0 ) ) {
    LOG_EXT_E( "Invalid user name or key specified" );
    goto finish;
  }
  ...
  if ( !ctx ) {
    LOG_EXT_E( "cannot create context" );
    goto finish;
  }
  ...
  if ( !session ) {
    LOG_EXT_E( "cannot create client session" );
    goto finish;
  }
  ...
  LOG_EXT_D( "sending CoAP request:");
  if (coap_get_log_level() < LOG_DEBUG)
    coap_show_pdu(LOG_INFO, pdu);
  ...
  wait_ms = wait_seconds * 1000;
  LOG_EXT_D( "timeout is set to %u seconds", wait_seconds);
    ...
    if ( result >= 0 ) {
      ...
      if ( obs_ms > 0 && !obs_ms_reset ) {
        if ( (unsigned)result >= obs_ms ) {
          LOG_EXT_D( "clear observation relationship" );
