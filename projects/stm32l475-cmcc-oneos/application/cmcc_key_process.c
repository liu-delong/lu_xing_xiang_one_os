...
#define DBG_ENABLE
#define DBG_SECTION_NAME               "CMCC_KEY"
#ifdef  CMCC_KEY_DEBUG
...
#define S2   GET_PIN(C, 13)
#define S3   GET_PIN(D, 10)
#define S4   GET_PIN(D, 9)
#define S5   GET_PIN(D, 8)
...
static void beep_ctrl(void *args)
{
    ...
    while(1)
    {
        rst = os_event_recv(&beep_event,
                        ...
                        OS_IPC_WAITING_FOREVER, (os_uint32_t *)&event);
...
void cmcc_key_init(void)
{
    ...
    os_event_init(&beep_event, "beep_event", OS_IPC_FLAG_FIFO);
    
    /* create  task 'cmcc_beep_ctl'*/
    cmcc_beep_ctl = os_task_create("beep_task",
                                    beep_ctrl,
                                    OS_NULL,
                                    CMCC_BEEP_TASK_STACK_SIZE, OS_TASK_PRIORITY_MAX / 2 - 4, 50);
