...
def PrepareUA(project, OS_ROOT, BSP_ROOT):
    with open('osua.py', 'w') as ua:
        # ua.write('import os\n')
        # ua.write('import sys\n')
        ua.write('\n')
        ...
        if len(CPPPATH):
            ...
            for path in paths:
                if PrefixPath(OS_ROOT, path):
                    CPPPATH += ['OS_ROOT + "/%s",' % _make_path_relative(OS_ROOT, path).replace('\\', '/')]
            ...
            CPPPATH.sort()
            ua.write('def GetCPPPATH(BSP_ROOT, OS_ROOT):\n')
            ua.write('\tCPPPATH=[\n')
            for path in CPPPATH:
                ua.write('\t\t%s\n' % path)
            ua.write('\t]\n\n')
