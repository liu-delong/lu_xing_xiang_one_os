...
def generate(env):
    assert(osconfig.CROSS_TOOL == 'clang-analyze')
    ...
    if osconfig.COMPILER_PATH:
        if not os.path.exists(osconfig.COMPILER_PATH):
            print
            print 'warning: osconfig.COMPILER_PATH(%s) does not exists.' % osconfig.COMPILER_PATH
            print
            return
        env.AppendENVPath('PATH', osconfig.COMPILER_PATH)
