...
$(eval C_SRC := $(C_SRC:$(OS_ROOT)/%=%)) \
$(eval COBJ := $(1:%.c=%.o)) \
$(eval COBJ := $(COBJ:$(BSP_ROOT)/%=$(BSP_BUILD_DIR)/%)) \
$(eval COBJ := $(COBJ:$(OS_ROOT)/%=$(RTT_BUILD_DIR)/%)) \
...
$(eval CXX_SRC := $(CXX_SRC:$(OS_ROOT)/%=%)) \
$(eval CXXOBJ := $(1:%.cpp=%.o)) \
$(eval CXXOBJ := $(CXXOBJ:$(BSP_ROOT)/%=$(BSP_BUILD_DIR)/%)) \
$(eval CXXOBJ := $(CXXOBJ:$(OS_ROOT)/%=$(RTT_BUILD_DIR)/%)) \
...
$(eval S_SRC := $(S_SRC:$(OS_ROOT)/%=%)) \
$(eval SOBJ := $(1:%.S=%.o)) \
$(eval SOBJ := $(SOBJ:$(BSP_ROOT)/%=$(BSP_BUILD_DIR)/%)) \
$(eval SOBJ := $(SOBJ:$(OS_ROOT)/%=$(RTT_BUILD_DIR)/%)) \
...
$(TARGET): $(OBJS) $(EXTERN_LIB)
	...
	@echo ------------------------------------------------
	@$(CROSS_COMPILE)objcopy -O binary $@ oneos.bin
	@$(CROSS_COMPILE)size $@
endif
