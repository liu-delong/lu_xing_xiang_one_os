...
class File(object):
    ...
    def c_data(self, prefix=''):
        ...
        head = 'static const cm_uint8_t %s[] = {\n' % \
                (prefix + self.c_name)
        tail = '\n};'
...
class Folder(object):
    ...
    def c_data(self, prefix=''):
        ...
        dtail = '\n};'
        body_fmt = '    {{{type}, "{name}", (cm_uint8_t *){data}, sizeof({data})/sizeof({data}[0])}}'
        body_fmt0= '    {{{type}, "{name}", CM_NULL, 0}}'
        # prefix of children
...
const struct romfs_dirent {name} = {{
    ROMFS_DIRENT_DIR, "/", (cm_uint8_t *){rootdirent}, sizeof({rootdirent})/sizeof({rootdirent}[0])
}};
'''
