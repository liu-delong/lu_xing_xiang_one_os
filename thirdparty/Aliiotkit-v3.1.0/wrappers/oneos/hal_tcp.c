...
uintptr_t HAL_TCP_Establish(const char *host, uint16_t port)
{
    ...
    LOG_EXT_D("establish tcp connection with server(host=%s port=%d)", host, port);
    ...
    if (-1 == rc)
    {
        LOG_EXT_E("fail to establish tcp");
    }
    else
    {
        LOG_EXT_D("success to establish tcp, fd=%d", rc);
    }
...
int32_t HAL_TCP_Read(uintptr_t fd, char *buf, uint32_t len, uint32_t timeout_ms)
{
    ...
    do
    {
        ...
        if (ret > 0)
        {
            ...
            {
                LOG_EXT_E("connection is closed");
                err_code = -1;
                break;
            }
            else
