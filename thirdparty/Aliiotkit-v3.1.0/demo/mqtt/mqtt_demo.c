...
static void example_message_arrive(void *pcontext, void *pclient, iotx_mqtt_event_msg_pt msg)
{
    ...
    case IOTX_MQTT_EVENT_PUBLISH_RECEIVED:
        ...
        LOG_EXT_I("Message Arrived:");
        LOG_EXT_I("Topic  : %.*s", topic_info->topic_len, topic_info->ptopic);
        LOG_EXT_I("Payload: %.*s", topic_info->payload_len, topic_info->payload);
        LOG_EXT_I("\n");
        break;
    default:
        break;
...
static int example_publish(void *handle)
{
    ...
    if (res < 0)
    {
        LOG_EXT_E("publish failed, res = %d", res);
        HAL_Free(topic);
        return -1;
    }
...
{
    LOG_EXT_I("msg->event_type : %d", msg->event_type);
}
...
int                everything_state_handle(const int state_code, const char *state_message)
{
    ...
    */
    LOG_EXT_I("recv -0x%04X(%s)", -state_code, state_message);
    return 0;
}
