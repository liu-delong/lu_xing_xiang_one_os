...
/* support both enable and disable "OS_USING_POSIX" */
#include <netdb.h>
...
static int webclient_read_line(struct webclient_session *session, char *buffer, int size)
{
    ...
    if (count > size)
    {
        LOG_EXT_E("read line failed. The line data length is out of buffer size(%d)!", count);
                                     ...
                                     const char *url, char **request)
{
    ...
    LOG_EXT_D("host address: %s , port: %s", session->host, port_str);
    ...
    /* resolve the host name. */
    {
        ...
        if(session->tls_session)
        {
            ...
            if (ret != 0)
            {
                LOG_EXT_E("getaddrinfo err: %d '%s'", ret, session->host);
                rc = -WEBCLIENT_ERROR;          
            }
...
static int webclient_open_tls(struct webclient_session *session, const char *URI)
{
    ...
    if (session->tls_session == OS_NULL)
    {
    ...
    if(session->tls_session->buffer == OS_NULL)
    {
        LOG_EXT_E("no memory for tls_session buffer!");
        return -WEBCLIENT_ERROR;
    }

    if((tls_ret = mbedtls_client_init(session->tls_session, (void *)pers, strlen(pers))) < 0)
    {
        LOG_EXT_E("initialize https client failed return: -0x%x.", -tls_ret);
        return -WEBCLIENT_ERROR;
    }
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    if (strncmp(URI, "https://", 8) == 0)
    {
        ...
        {
            LOG_EXT_E("connect failed, https client open URI(%s) failed!", URI);
            return -WEBCLIENT_ERROR;
        }
#else
        LOG_EXT_E("not support https connect, please enable webclient https configure!");
        rc = -WEBCLIENT_ERROR;
        goto __exit;
#endif
    }
    ...
    if (rc != WEBCLIENT_OK)
    {
        LOG_EXT_E("connect failed, resolve address error(%d).", rc);
        goto __exit;
    }
    ...
    if (session->tls_session)
    {
        ...
        {
            LOG_EXT_E("connect failed, https client context return: -0x%x", -tls_ret);
            return -WEBCLIENT_ERROR;
        }

        if ((tls_ret = mbedtls_client_connect(session->tls_session)) < 0)
        {
            LOG_EXT_E("connect failed, https client connect return: -0x%x", -tls_ret);
...

    {
        ...
        if (socket_handle < 0)
        {
            LOG_EXT_E("connect failed, create socket(%d) error.", socket_handle);
        ...
        {
            /* connect failed */
            LOG_EXT_E("connect failed, connect socket(%d) error.", socket_handle);
            rc = -WEBCLIENT_CONNECT_FAILED;
            goto __exit;
        }
...
int webclient_header_fields_add(struct webclient_session *session, const char *fmt, ...)
{
    ...
    OS_ASSERT(session);
    OS_ASSERT(session->header->buffer);

    va_start(args, fmt);
    length = os_vsnprintf(session->header->buffer + session->header->length,
            session->header->size - session->header->length, fmt, args);
    if (length < 0)
    {
        LOG_EXT_E("add fields header data failed, return length(%d) error.", length);
        return -WEBCLIENT_ERROR;
    }
    va_end(args);
    ...
    {
        LOG_EXT_E("not enough header buffer size(%d)!", session->header->size);
        return -WEBCLIENT_ERROR;
    }
...
static int webclient_send_header(struct webclient_session *session, int method)
{
    ...
    if (session->header->length == 0)
    {
        ...
        if (webclient_header_fields_add(session, "Host: %s\r\n", session->host) < 0)
            return -WEBCLIENT_NOMEM;
        if (webclient_header_fields_add(session, "User-Agent: CMCC IOT HTTP Agent\r\n\r\n") < 0)
            return -WEBCLIENT_NOMEM;
    ...
    else
    {
        if (method != WEBCLIENT_USER_METHOD)
        {
            ...
            {
                char *header_buffer = OS_NULL;
                int length = 0;

                header_buffer = web_strdup(session->header->buffer);
                if (header_buffer == OS_NULL)
                {
                    LOG_EXT_E("no memory for header buffer!");
                    rc = -WEBCLIENT_NOMEM;
                    goto __exit;
                }
                ...
                if (method == WEBCLIENT_GET)
                    length = os_snprintf(session->header->buffer, session->header->size, "GET %s HTTP/1.1\r\n%s",
                            session->req_url ? session->req_url : "/", header_buffer);
                else if (method == WEBCLIENT_POST)
                    length = os_snprintf(session->header->buffer, session->header->size, "POST %s HTTP/1.1\r\n%s",
                            session->req_url ? session->req_url : "/", header_buffer);
                session->header->length = length;
            ...
            /* header data end */
            os_snprintf(session->header->buffer + session->header->length, session->header->size, "\r\n");
            session->header->length += 2;

            /* check header size */
            if (session->header->length > session->header->size)
            {
                LOG_EXT_E("send header failed, not enough header buffer size(%d)!", session->header->size);
...
int webclient_handle_response(struct webclient_session *session)
{
    ...
    if (mime_ptr == OS_NULL)
    {
        LOG_EXT_E("no memory for get http status code buffer!");
        return -WEBCLIENT_NOMEM;
    }
    ...
    if (strstr(mime_ptr, "HTTP/1."))
    {
        ...
        session->resp_status = (int) strtol(ptr, OS_NULL, 10);
    }

    /* get content length */
    if (webclient_header_fields_get(session, "Content-Length") != OS_NULL)
    {
    ...
    if (transfer_encoding && strcmp(transfer_encoding, "chunked") == 0)
    {
        ...
        session->chunk_sz = strtol(line, OS_NULL, 16);
...
struct webclient_session *webclient_session_create(size_t header_sz)
{
    ...
    if (session == OS_NULL)
    {
        LOG_EXT_E("webclient create failed, no memory for webclient session!");
        return OS_NULL;
    }
    ...
    if (session->header == OS_NULL)
    {
        LOG_EXT_E("webclient create failed, no memory for session header!");
        web_free(session);
        session = OS_NULL;
        return OS_NULL;
    }
    ...
    if (session->header->buffer == OS_NULL)
    {
        LOG_EXT_E("webclient create failed, no memory for session header buffer!");
...
int webclient_get(struct webclient_session *session, const char *URI)
{
    ...
    if (resp_status > 0)
    {
        ...
        {
            LOG_EXT_E("get failed, handle response(%d) error!", resp_status);
            return resp_status;
        }
    }
...
int webclient_post(struct webclient_session *session, const char *URI, const char *post_data)
{
    ...
    if (post_data)
    {
        ...
        if (resp_status != 200)
        {
            LOG_EXT_E("post failed, handle response(%d) error.", resp_status);
            return resp_status;
        }
    }
...
int webclient_read(struct webclient_session *session, unsigned char *buffer, size_t length)
{
    ...
    if (session->chunk_sz)
    {
        ...
        if (bytes_read <= 0)
        {
            if (os_get_errno() == EWOULDBLOCK || os_get_errno() == EAGAIN)
            {
                /* recv timeout */
    ...
    do
    {
        ...
        if (bytes_read <= 0)
        {
            ...
            if(session->tls_session && bytes_read == MBEDTLS_ERR_SSL_WANT_READ)
                continue;
#endif  
            LOG_EXT_E("receive data error(%d).", bytes_read);
            ...
            else
            {
                ...
                {
                    /* recv timeout */
                    LOG_EXT_E("receive data timeout.");
                    return -WEBCLIENT_TIMEOUT;
                }
                else
                {
...
int webclient_request(const char *URI, const char *header, const char *post_data, unsigned char **response)
{
    ...
    if (post_data && header == OS_NULL)
    {
        LOG_EXT_E("request post failed, post input header cannot be empty.");
        return -WEBCLIENT_ERROR;
    }

    if (post_data == OS_NULL && response == OS_NULL)
    {
        LOG_EXT_E("request get failed, get response data cannot be empty.");
        return -WEBCLIENT_ERROR;
    }

    if (post_data == OS_NULL)
    {
