...
/*! Parameter check */
#define DDL_ASSERT(x)       do { assert((x) > 0u); } while (0)
/*******************************************************************************
...
en_result_t Dma_InitChannel(en_dma_channel_t enCh, stc_dma_config_t* pstcConfig)
{
  ...
  DDL_ASSERT(IS_VALID_ADDR_MODE(pstcConfig->enSrcAddrMode));
  DDL_ASSERT(IS_VALID_ADDR_MODE(pstcConfig->enDstAddrMode));
...
{
  DDL_ASSERT(IS_VALID_PRIO_MODE(enPrio));
