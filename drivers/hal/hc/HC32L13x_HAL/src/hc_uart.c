...
uint16_t Uart_CalScnt(uint8_t u8Idx,stc_uart_baud_t *pstcBaud)
{
    ...
    if((UartMode1 == pstcBaud->enRunMode)||(UartMode3 == pstcBaud->enRunMode))
    {
        ...
        u16Scnt = pstcBaud->u32Pclk/(pstcBaud->u16Baud*u8Over);
    }
    else
    {
        if(0 == u8Over)
 ...
 ** \retval 波特率
 ******************************************************************************/
uint16_t Uart_GetBaud(uint8_t u8Idx,uint8_t u8Mode,uint32_t u32Pclk)
{
    stc_uart_instance_data_t *pstcData = NULL;
	uint16_t u16Baud = 0;
    uint8_t u8Over = 0;
    ...
    switch(u8Mode)
    {
        ...
        case UartMode3:
            ...
            u16Baud = u32Pclk/(u8Over*u16Scnt);
            break;
        case UartMode2:
            if(0 == pstcData->pstcInstance->SCON_f.OVER)
            {
