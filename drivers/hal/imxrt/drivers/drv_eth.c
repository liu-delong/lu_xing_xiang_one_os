 ...
 * @file        drv_eth.c
 *
 * @brief       This file implements eth driver for imxrt.
 *
 * @revision
 * Date         Author          Notes
...
struct rt_imxrt_eth
{
    ...
    os_bool_t tx_is_waiting;
    struct os_semaphore tx_wait;
...
ALIGN(ENET_BUFF_ALIGNMENT) os_uint8_t g_txDataBuff[ENET_TXBD_NUM][RT_ALIGN(ENET_TXBUFF_SIZE, ENET_BUFF_ALIGNMENT)];

ALIGN(ENET_BUFF_ALIGNMENT) enet_rx_bd_struct_t g_rxBuffDescrip[ENET_RXBD_NUM] SECTION("NonCacheable");
ALIGN(ENET_BUFF_ALIGNMENT) os_uint8_t g_rxDataBuff[ENET_RXBD_NUM][RT_ALIGN(ENET_RXBUFF_SIZE, ENET_BUFF_ALIGNMENT)];
...
{
    if (eth->tx_is_waiting == OS_TRUE)
    {
        eth->tx_is_waiting = OS_FALSE;
        os_sem_post(&eth->tx_wait);
    }
}
...
static os_err_t rt_imxrt_eth_init(os_device_t dev)
{
...
static os_err_t rt_imxrt_eth_open(os_device_t dev, os_uint16_t oflag)
{
    dbg_log(DBG_LOG, "rt_imxrt_eth_open...\n");
    return OS_EOK;
}

static os_err_t rt_imxrt_eth_close(os_device_t dev)
{
    dbg_log(DBG_LOG, "rt_imxrt_eth_close...\n");
    return OS_EOK;
}

static os_size_t rt_imxrt_eth_read(os_device_t dev, os_off_t pos, void *buffer, os_size_t size)
{
...
static os_size_t rt_imxrt_eth_write(os_device_t dev, os_off_t pos, const void *buffer, os_size_t size)
{
...
static os_err_t rt_imxrt_eth_control(os_device_t dev, int cmd, void *args)
{
    ...
    case NIOCTL_GADDR:
        /* get mac address */
        if (args) os_memcpy(args, imxrt_eth_device.dev_addr, 6);
...
/* transmit packet. */
os_err_t rt_imxrt_eth_tx(os_device_t dev, struct pbuf *p)
{
	os_err_t result = OS_EOK;
	enet_handle_t * enet_handle = &imxrt_eth_device.enet_handle;

    OS_ASSERT(p != NULL);
    OS_ASSERT(enet_handle != OS_NULL);
    ...
    do
    {
        ...
        {
            imxrt_eth_device.tx_is_waiting = OS_TRUE;
            os_sem_wait(&imxrt_eth_device.tx_wait, OS_IPC_WAITING_FOREVER);
        }
...
/* reception packet. */
struct pbuf *rt_imxrt_eth_rx(os_device_t dev)
{
    uint32_t length = 0;
    status_t status;
        ...
        if ((status == kStatus_Success) && (link != new_link))
        {
            ...
            if (link)   // link up
            {
                        ...
                        || (imxrt_eth_device.duplex != (enet_mii_duplex_t)duplex))
                {
                    ...
                    rt_imxrt_eth_init((os_device_t)&imxrt_eth_device);
                }
                else
                {
                ...
                eth_device_linkchange(&imxrt_eth_device.parent, OS_TRUE);
            }
            else
            {
                dbg_log(DBG_LOG, "link down.\n");
                eth_device_linkchange(&imxrt_eth_device.parent, OS_FALSE);
            }
        }

        os_thread_delay(OS_TICK_PER_SECOND * 2);
    }
}
...
static int rt_hw_imxrt_eth_init(void)
{
    ...
    imxrt_eth_device.parent.parent.user_data  = OS_NULL;
    ...
    /* init tx semaphore */
    rt_sem_init(&imxrt_eth_device.tx_wait, "tx_wait", 0, OS_IPC_FLAG_FIFO);
...
void phy_read(uint32_t phyReg)
{
    ...
    {
        os_kprintf("PHY_Read: %02X --> %08X", phyReg, data);
    }
    else
    {
        os_kprintf("PHY_Read: %02X --> faild", phyReg);
    }
}
...
void phy_write(uint32_t phyReg, uint32_t data)
{
    ...
    if (kStatus_Success == status)
    {
        os_kprintf("PHY_Write: %02X --> %08X\n", phyReg, data);
    }
    else
    {
        os_kprintf("PHY_Write: %02X --> faild\n", phyReg);
    }
}
...
void phy_dump(void)
{
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        if (kStatus_Success != status)
        {
            os_kprintf("phy_dump: %02X --> faild", i);
            break;
        }

        if (i % 8 == 7)
        {
            os_kprintf("%02X --> %08X ", i, data);
        }
        else
        {
            os_kprintf("%02X --> %08X\n", i, data);
...
void enet_buf_info(void)
{
    ...
    {
        os_kprintf("%d: length: %-8d, control: %04X, buffer:%p\n",
                   i,
