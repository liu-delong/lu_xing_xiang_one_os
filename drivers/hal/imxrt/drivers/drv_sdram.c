 ...
 * @file        drv_sdram.c
 *
 * @brief       This file implements sdram driver for imxrt.
 *
 * @revision
 * Date         Author          Notes
...
int rt_hw_sdram_Init(void)
{
    ...
    os_uint32_t clockFrq = CLOCK_GetFreq(kCLOCK_SemcClk);
...
os_uint32_t sdram_writeBuffer[SEMC_DATALEN];
os_uint32_t sdram_readBuffer[SEMC_DATALEN];

/* read write 32bit test */
void sdram_test(void)
{
    os_uint32_t index;
    os_uint32_t datalen = SEMC_DATALEN;
    os_uint32_t *sdram = (os_uint32_t *)SDRAM_BANK_ADDR; /* SDRAM start address. */
    bool result = true;
