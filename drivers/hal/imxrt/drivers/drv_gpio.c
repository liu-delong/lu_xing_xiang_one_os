...
static void imxrt_pin_mode(os_device_t *dev, os_base_t pin, os_base_t mode)
{
    ...
    {
        LOG_EXT_D("invalid pin, os pin: %d,port: %d,pin: %d \n", pin,port + 1,pin_num);
        return;
    }
...
static int imxrt_pin_read(os_device_t *dev, os_base_t pin)
{
    ...
    if (PIN_INVALID_CHECK(port, pin_num))
    {
        LOG_EXT_D("invalid pin,rtt pin: %d,port: %d,pin: %d \n", pin,port + 1,pin_num);
        return value;
    }
...
static os_err_t imxrt_pin_irq_enable(struct os_device *device, os_base_t pin, os_uint32_t enabled)
{
    ...
    if (PIN_INVALID_CHECK(port, pin_num))
    {
        LOG_EXT_D("invalid pin, pin: %d,port: %d,pin: %d \n", pin,port + 1,pin_num);
        return OS_ENOSYS;
    }

    if (hdr_tab[pin].pin == -1)
    {
        LOG_EXT_D("os pin: %d callback function not initialized!\n", pin);
        return OS_ENOSYS;
    }
