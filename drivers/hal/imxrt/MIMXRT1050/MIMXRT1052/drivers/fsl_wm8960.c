...
status_t WM8960_Deinit(wm8960_handle_t *handle)
{
    ...
    // TODO: i2c deinit
    //CODEC_I2C_Deinit(handle->i2cHandle);
...
status_t WM8960_WriteReg(wm8960_handle_t *handle, uint8_t reg, uint16_t val)
{
    ...
    transfer.slaveAddress   = handle->config->slaveAddress;
    transfer.direction      = kLPI2C_Write;
    transfer.subaddress     = cmd;
    transfer.subaddressSize = 1U;
    transfer.data           = (uint8_t *)&buff;
    transfer.dataSize       = 2U;
    
    result = LPI2C_MasterTransferBlocking(handle->config->i2cBase, &transfer);
