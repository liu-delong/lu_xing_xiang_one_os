...
static os_err_t os_stm32_eth_init(os_device_t *dev)
{
    ...
    {
        LOG_EXT_E("eth hardware init failed");
    }
    else
    {
        LOG_EXT_D("eth hardware init success");
    }
    ...
    {
        LOG_EXT_D("emac hardware start");
    }
    else
    {
        LOG_EXT_E("emac hardware start faild");
        return OS_ERROR;
    }
...
static os_err_t os_stm32_eth_open(os_device_t *dev, os_uint16_t oflag)
{
    LOG_EXT_D("emac open");
    return OS_EOK;
}

static os_err_t os_stm32_eth_close(os_device_t *dev)
{
...
os_err_t os_stm32_eth_tx(os_device_t *dev, struct pbuf *p)
{
    ...
    for (q = p; q != NULL; q = q->next)
    {
        ...
        while ((byteslefttocopy + bufferoffset) > ETH_TX_BUF_SIZE)
        {
            ...
            if ((DmaTxDesc->Status & ETH_DMATXDESC_OWN) != (uint32_t)RESET)
            {
                LOG_EXT_E("dma tx desc buffer is not valid");
                ret = ERR_USE;
                goto error;
            }
...
struct pbuf *os_stm32_eth_rx(os_device_t *dev)
{
    ...
    LOG_EXT_D("receive frame len : %d", len);
...
OS_DRIVER_DEFINE(stm32_eth_driver, "4");
