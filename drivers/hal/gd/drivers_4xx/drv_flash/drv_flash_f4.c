...
#include <drv_log.h>
#define FLASH_PAGE_SIZE           ((uint32_t)(128*1024))
/**
 ***********************************************************************************************************************
...
int gd32_flash_erase(os_uint32_t addr, size_t size)
{
    ...
    for(erase_cnt = 0; erase_cnt < page_num; erase_cnt++){
        fmc_erase_sector_by_address(page_idx + (FLASH_PAGE_SIZE * erase_cnt));
    }
...
int gd32_flash_write(os_uint32_t addr, const os_uint8_t *buf, size_t size)
{
    ...
    end_sector_info = fmc_sector_info_get(addr + size);
    /* erase sector */
    for(i = start_sector_info.sector_name; i <= end_sector_info.sector_name; i++){
        ...
        if(FMC_READY != fmc_sector_erase(sector_num)){
            os_kprintf("erase faild before write!\n");
        }
    }
#endif
...
#define GD32_FLASH_PAGE_SIZE   (128)
