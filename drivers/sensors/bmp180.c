...
#define DBG_EXT_TAG "sensor.adi.bmp180"
...
static bmp180_info_t *bmp180_init(const char *bus_name, os_uint16_t addr)
{
    ...
    if (bmp180 == OS_NULL)
    {
        LOG_EXT_E("bmp180 amlloc faile");
        return NULL;
    }

    LOG_EXT_I("bmp180:[%s][0x%02x]", bus_name, addr);

    bmp180->i2c.bus = os_i2c_bus_device_find(bus_name);
    if (bmp180->i2c.bus == NULL)
    {
        LOG_EXT_E("bmp180 i2c invalid.");
        return NULL;
    }
    ...
    LOG_EXT_I("bmp180 devid:0x%02x", devid);
    if (devid != 0x55)
    {
        LOG_EXT_E("bmp180 devid invalid.");
        os_free(bmp180);
        return NULL;
    }
...
static int os_hw_bmp180_init(void)
{
    ...
    result = os_hw_sensor_register(&bmp180->sensor_temp, "bmp180", OS_DEVICE_FLAG_RDWR, OS_NULL);
    if (result != OS_EOK)
    {
    ...
    result = os_hw_sensor_register(&bmp180->sensor_baro, "bmp180", OS_DEVICE_FLAG_RDWR, OS_NULL);
    if (result != OS_EOK)
    {
        LOG_EXT_E("device register err code: %d", result);
        goto __exit;
    ...
    result = os_hw_sensor_register(&bmp180->sensor_alti, "bmp180", OS_DEVICE_FLAG_RDWR, OS_NULL);
    if (result != OS_EOK)
    {
        LOG_EXT_E("device register err code: %d", result);
        goto __exit;
...
OS_DEVICE_INIT(os_hw_bmp180_init);
