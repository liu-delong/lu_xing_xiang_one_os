...
#define DBG_EXT_TAG "sensor.adi.ak8963"
...
static void mpu9250_bypass(void)
{
    ...
    if (i2c.bus == NULL)
    {
        LOG_EXT_E("ak8963(mpu9250) i2c invalid.");
        return;
    }
...
static ak8963_info_t *ak8963_Init(const char *bus_name, os_uint16_t addr)
{
    ...
    LOG_EXT_I("ak8963:[%s][0x%02x]", bus_name, addr);

    ak8963 = os_calloc(1, sizeof(ak8963_info_t));
    if (ak8963 == OS_NULL)
    {
        LOG_EXT_E("ak8963 amlloc faile");
        return NULL;
    }
    ...
    LOG_EXT_I("ak8963 devid:0x%02x", devid);
    if (devid != AK8963_ID)
    {
        LOG_EXT_E("ak8963 devid invalid:0x%02x", devid);
        os_free(ak8963);
        return NULL;
    }
...
static uint8_t ak8963_get_value(ak8963_info_t *ak8963)
{
    ...
    LOG_EXT_D("(%d, %d, %d).", (int)ak8963->mx, (int)ak8963->my, (int)ak8963->mz);
...
static int os_hw_ak8963_init(void)
{
    ...
    result = os_hw_sensor_register(&ak8963->sensor, "ak8963", OS_DEVICE_FLAG_RDWR, OS_NULL);
    if (result != OS_EOK)
    {
...
OS_INIT_EXPORT(os_hw_ak8963_init, "6");
