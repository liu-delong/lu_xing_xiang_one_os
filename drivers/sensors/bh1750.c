...
static float read_bh1750(bh1750_info_t *bh1750)
{
    ...
    LOG_EXT_I("BH1750 DATA is: %d LUX", (int)value_lx);
...
static bh1750_info_t *bh1750_Init(const char *bus_name, os_uint16_t addr)
{
    ...
    LOG_EXT_I("bh1750:[%s][0x%02x]", bus_name, addr);

    bh1750 = os_calloc(1, sizeof(bh1750_info_t));
    if (bh1750 == OS_NULL)
    {
        LOG_EXT_E("bh1750 amlloc faile");
        return NULL;
    }
...
static int os_hw_bh1750_init(void)
{
    ...
    result = os_hw_sensor_register(&bh1750->sensor, "bh1750", OS_DEVICE_FLAG_RDWR, OS_NULL);
    if (result != OS_EOK)
    {
...
OS_DEVICE_INIT(os_hw_bh1750_init);
