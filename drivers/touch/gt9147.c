...
static int os_hw_gt9147_init(const char *name, struct os_touch_config *cfg)
{
    ...
    {
        LOG_EXT_E("Can't find device\n");
        return OS_ERROR;
    }

    if (os_device_open((os_device_t *)gt9147_client->bus, OS_DEVICE_FLAG_RDWR) != OS_EOK)
    {
    ...
    os_hw_touch_register(touch_device, name, OS_DEVICE_FLAG_INT_RX, OS_NULL);

    LOG_EXT_I("touch device gt9147 init success\n");
