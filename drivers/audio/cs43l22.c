...
#define DBG_COLOR
#define DBG_SECTION_NAME "CS43L22"
...
uint32_t cs43l22_init(struct cs43l22_dev *cs43l22, uint16_t OutputDevice, uint8_t Volume, uint32_t AudioFreq)
{
    ...
    if (id != CS43L22_ID)
    {
        LOG_EXT_E("cs43l22 id: %02x invalid, expect: %02x", id, CS43L22_ID);
        return (uint32_t)-1;
    }
    ...
    if (counter == 0)
    {
        LOG_EXT_I("cs43l22 init success");
    }
...
int os_hw_audio_player_init(void)
{
    ...
    cs43l22_player->tx_fifo = tx_fifo;
    ...
    cs43l22_player->sai = (os_device_sai_t *)os_device_find(BSP_SAI_BLOCK);
    if (cs43l22_player->sai == OS_NULL)
    {
    ...
    /* cs43l22 device */
    cs43l22->i2c.bus = os_i2c_bus_device_find(BSP_CS43L22_I2C_BUS);
    if (cs43l22->i2c.bus == NULL)
    {
        LOG_EXT_E("cs43l22 i2c invalid.");
        os_free(cs43l22_player);
        return -1;
    }
    ...
    os_audio_player_register(&cs43l22_player->audio, "audio0", OS_DEVICE_FLAG_WRONLY, cs43l22_player);
