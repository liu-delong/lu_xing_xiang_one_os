...
void speaker_entry(void *parameter)
{
    ...
    speaker.buffer = os_malloc(AUDIO_BUFFER_SZ);
    ...
    {
        LOG_EXT_E("can't find device:%s", SPEAKER_DEVICE_NAME);
        goto __exit;
    }
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (e & EVENT_AUDIO_DATA)
            {
                ...
                os_device_write(speaker.dev, 0, speaker.buffer + index, AUDIO_BUFFER_SZ / 2);
            }
