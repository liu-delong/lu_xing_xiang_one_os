...
static os_err_t _ep_out_handler(ufunction_t func, os_size_t size)
{
    ...
    if (size != 0)
    {
        ...
        report.size = size - 1;
        os_mq_send(&data->hid_mq, (void *)&report, sizeof(report), OS_IPC_WAITING_NO);
    }
...
static os_err_t _function_enable(ufunction_t func)
{
    ...
    */
    ...
    {
        data->ep_out->buffer = os_malloc(HID_RX_BUFSIZE);
    }
    data->ep_out->request.buffer   = data->ep_out->buffer;
    data->ep_out->request.size     = EP_MAXPACKET(data->ep_out);
    data->ep_out->request.req_type = UIO_REQUEST_READ_BEST;
...
static void hid_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        if (os_mq_recv(&hiddev->hid_mq, &report, sizeof(report), OS_IPC_WAITING_FOREVER, &recv_len) != OS_EOK)
            continue;
...
ufunction_t os_usbd_function_hid_create(udevice_t device)
{
    ...
    data = (struct hid_s *)os_malloc(sizeof(struct hid_s));
...
OS_PREV_INIT(os_usbd_hid_class_register);
