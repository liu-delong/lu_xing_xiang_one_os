...
static os_err_t  os_msd_open(os_device_t *dev, os_uint16_t oflag);
static os_err_t  os_msd_close(os_device_t *dev);
static os_size_t os_msd_write(os_device_t *dev, os_off_t pos, const void *buffer, os_size_t size);
static os_size_t os_msd_read(os_device_t *dev, os_off_t pos, void *buffer, os_size_t size);
static os_size_t os_msd_sdhc_read(os_device_t *dev, os_off_t pos, void *buffer, os_size_t size);
static os_size_t os_msd_sdhc_write(os_device_t *dev, os_off_t pos, const void *buffer, os_size_t size);
static os_err_t  os_msd_control(os_device_t *dev, int cmd, void *args);

static os_err_t MSD_take_owner(struct os_spi_device *spi_device)
{
    os_err_t result;

    result = os_mutex_lock(&(spi_device->bus->lock), OS_IPC_WAITING_FOREVER);
    if (result == OS_EOK)
    {
...
const static struct os_device_ops msd_sdhc_ops = 
{
    os_msd_init,
    os_msd_open,
    os_msd_close,
...
static os_size_t os_msd_read(os_device_t *dev, os_off_t pos, void *buffer, os_size_t size)
{
    ...
    if (size == 1)
    {
        ...
        result = _send_cmd(msd->spi_device,
                           READ_SINGLE_BLOCK,
                           pos * msd->geometry.bytes_per_sector,
                           0x00,
                           response_r1,
                           response);
        if ((result != OS_EOK) || (response[0] != MSD_RESPONSE_NO_ERROR))
...
static os_size_t os_msd_write(os_device_t *dev, os_off_t pos, const void *buffer, os_size_t size)
{
    ...
    if (size == 1)
    {
        os_spi_take(msd->spi_device);
        result =
            _send_cmd(msd->spi_device, WRITE_BLOCK, pos * msd->geometry.bytes_per_sector, 0x00, response_r1, response);
        if ((result != OS_EOK) || (response[0] != MSD_RESPONSE_NO_ERROR))
        {
...
os_err_t msd_init(const char *sd_device_name, const char *spi_device_name)
{
    ...
    _msd_device.parent.type = OS_DEVICE_TYPE_BLOCK;
