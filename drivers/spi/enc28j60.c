...
static struct os_event                           tx_event;
...
#define enc28j60_lock(dev)   os_mutex_lock(&((struct net_device *)dev)->lock, OS_IPC_WAITING_FOREVER);
...
static os_err_t enc28j60_init(os_device_t *dev)
{
    ...
    os_task_mdelay(OS_TICK_PER_SECOND / 50); /* delay 20ms */
...
static os_err_t enc28j60_open(os_device_t *dev, uint16_t oflag)
{
    return OS_EOK;
}

static os_err_t enc28j60_close(os_device_t *dev)
{
    return OS_EOK;
}
...
os_err_t enc28j60_attach(const char *spi_device_name)
{
    ...
    os_event_init(&tx_event, "eth_tx", OS_IPC_FLAG_FIFO);
    os_mutex_init(&enc28j60_dev.lock, "enc28j60", OS_IPC_FLAG_FIFO, OS_FALSE);
...
OS_INIT_EXPORT(os_hw_enc28j60_init, "5");
