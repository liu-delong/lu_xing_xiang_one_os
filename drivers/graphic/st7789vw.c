...
static os_err_t lcd_write_cmd(const os_uint8_t cmd)
{
    ...
    if (len != 1)
    {
        LOG_EXT_I("lcd_write_cmd error. %d", len);
        return OS_ERROR;
    }
    else
    {
...
static os_err_t lcd_write_data(const os_uint8_t data)
{
    ...
    if (len != 1)
    {
        LOG_EXT_I("lcd_write_data error. %d", len);
        return OS_ERROR;
    }
    else
    {
...
static os_err_t lcd_write_half_word(const os_uint16_t da)
{
    ...
    if (len != 2)
    {
        LOG_EXT_I("lcd_write_half_word error. %d", len);
        return OS_ERROR;
    }
    else
    {
...
void lcd_clear(os_uint16_t color)
{
    ...
    /* 5760 = 240*240/20 */
    buf = os_malloc(LCD_CLEAR_SEND_NUMBER);
    if (buf)
    {
...
static void lcd_show_char(os_uint16_t x, os_uint16_t y, os_uint8_t data, os_uint32_t size)
{
    ...
    if (size == 16)
    {
        ...
        font_buf = (os_uint8_t *)os_malloc(size * size);
        if (!font_buf)
        {
            /* fast show char */
...
#endif
    {
        LOG_EXT_E("There is no any define ASC2_1208 && ASC2_2412 && ASC2_2416 && ASC2_3216 !");
    }
}
...
#define LCD_STRING_BUF_LEN 128
    ...
    {
        LOG_EXT_E("font size(%d) is not support!", size);
        return OS_ERROR;
    }
