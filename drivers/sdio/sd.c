...
static os_int32_t mmcsd_parse_csd(struct os_mmcsd_card *card)
{
    ...
    case 1:
        ...
        break;
    default:
        LOG_EXT_E("unrecognised CSD structure version %d!", csd->csd_structure);

        return OS_ERROR;
    }
    LOG_EXT_I("SD card capacity %d KB.", card->card_capacity);
    return 0;
}
...
static os_int32_t mmcsd_switch(struct os_mmcsd_card *card)
{
    ...
    buf = (os_uint8_t *)os_malloc(64);
    if (!buf)
    {
        LOG_EXT_E("alloc memory failed!");
    ...
    if ((buf[16] & 0xF) != 1)
    {
        LOG_EXT_I("switching card to high speed failed!");
        goto err;
    }
...
static os_int32_t mmcsd_sd_init_card(struct os_mmcsd_host *host, os_uint32_t ocr)
{
    ...
    card = (struct os_mmcsd_card *)os_malloc(sizeof(struct os_mmcsd_card));
    if (!card)
    {
        LOG_EXT_E("malloc card failed!");
        err = OS_ENOMEM;
        goto err;
    }
...
os_int32_t init_sd(struct os_mmcsd_host *host, os_uint32_t ocr)
{
    ...
    if (ocr & VDD_165_195)
    {
        LOG_EXT_I(" SD card claims to support the "
