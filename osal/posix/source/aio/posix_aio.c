 ...
 * @file        posix_aio.c
 *
 * @brief       This file implements the asynchronous IO API.
 *
 * @revision
 * Date         Author          Notes
...
struct os_workqueue* g_aio_queue = NULL;
...
int aio_cancel(int fd, struct aiocb *cb)
{
    ...
    ret = os_workqueue_cancel_work_sync(g_aio_queue, &cb->aio_work);
    if (ret == OS_EOK)
    {
        errno = -ECANCELED;
        return -1;
 ...
 */
static void aio_fync_work(struct os_work *work, void *work_data)
{
    int           result;
    os_ubase_t    level;
    struct aiocb *cb;

    cb = (struct aiocb *)work_data;
...
int aio_fsync(int op, struct aiocb *cb)
{
    ...
    os_work_init(&cb->aio_work, aio_fync_work, cb);
    os_workqueue_submit_work(g_aio_queue, &cb->aio_work, 0);

    return 0;
}

static void aio_read_work(struct os_work *work, void *work_data)
{
    int          len;
    os_ubase_t   level;
    uint8_t      *buf_ptr;
...
int aio_read(struct aiocb *cb)
{
    ...
    /* En-queue read work */
    os_work_init(&cb->aio_work, aio_read_work, cb);
    os_workqueue_submit_work(g_aio_queue, &cb->aio_work, 0);
...
static void aio_write_work(struct os_work *work, void *work_data)
{
    int           len;
    int           oflags;
    int           level;
...
int aio_write(struct aiocb *cb)
{
    ...
    os_work_init(&cb->aio_work, aio_write_work, cb);
    os_workqueue_submit_work(g_aio_queue, &cb->aio_work, 0);
...
{
    g_aio_queue = os_workqueue_create("aio", 2048, OS_TASK_PRIORITY_MAX / 2);
    
    OS_ASSERT(NULL != g_aio_queue);

    return 0;
}
OS_CMPOENT_INIT(aio_system_init);
